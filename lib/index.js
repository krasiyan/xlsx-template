...
module.exports = (function() {
    ...
    */
    ...
    /**
     * pipe the new .xlsx file to a stream
     */
    Workbook.prototype.pipe = function(stream) {
        var self = this;
        var newArchive = archiver('zip', {
          zlib: { level: 9 }
        });

        newArchive.on('warning', function (err) {
          throw err;
        })
        newArchive.on('error', function(err) {
          throw err;
        });

        newArchive.pipe(stream);

        Object.keys(this.archive.files).forEach(function (fileName) {
          newArchive.append(self.archive.files[fileName]._data, { name: fileName });
        })
